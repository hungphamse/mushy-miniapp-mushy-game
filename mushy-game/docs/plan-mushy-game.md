...
Level generation cron:
- Level content is generated by the companion `level-editor` deployment before players access it. Player requests must never generate missing daily levels.
- Normal daily generation runs through Vercel Cron on `level-editor/api/cron/generate-levels.js`.
- The same cron handler must also support a manually triggered backfill/generate call for a specific date, for example `POST /api/cron/generate-levels?date=YYYY-MM-DD`.
- The cron handler is idempotent: if a generated/published row already exists, it leaves it alone; if a custom row exists, it never overwrites it; if no row exists, it inserts the generated level.
- Launch setup must run the manual generate call immediately after applying `level-editor/migrations/001_level_catalog_schema.sql` so the migration-date level `001` exists even if the scheduled cron for that date already passed.
- The database trigger `daily_levels_set_level_number` assigns stored `level_number` during cron inserts, so the cron passes only `game_id`, `puzzle_date`, source/status, content, and generator metadata.
