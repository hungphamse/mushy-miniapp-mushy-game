...
- Per-length word lists from SCOWL (SCOWL-35 for answers, SCOWL-50 for valid guesses), pre-bundled, client-side validation
...
| `answer` | `string` | Uppercase ASCII English word of exactly `wordLength` letters; must exist in `ANSWER_LISTS[wordLength]` (SCOWL-35) |
...
|---|---|---|---|
| `ANSWER_LISTS[4–8]` | **35** — everyday vocabulary, words in at least 11 of 12 major dictionaries | Answer pool — what the cron and the companion `level-editor` pick from | Yes — LDNOOBWV2 applied at build time + manual spot-check |
| `VALID_GUESS_LISTS[4–8]` | **50** — medium, most dictionary words | Guess validation — what the renderer accepts as a valid submission | No — see note below |
...
Both lists are **pre-bundled** at `src/lib/data/wordLists.js` — no runtime fetch. The `ANSWER_LISTS` (SCOWL-35) are also used by the companion `level-editor` autocomplete feature (see §10.6).

```js
// src/lib/data/wordLists.js  (generated by scripts/build-word-lists.js)
export const ANSWER_LISTS = {
  4: [ /* SCOWL-35 filtered to length 4, LDNOOBWV2 applied */ ],
  5: [ /* SCOWL-35 filtered to length 5, LDNOOBWV2 applied */ ],
  6: [ /* SCOWL-35 filtered to length 6, LDNOOBWV2 applied */ ],
  7: [ /* SCOWL-35 filtered to length 7, LDNOOBWV2 applied */ ],
  8: [ /* SCOWL-35 filtered to length 8, LDNOOBWV2 applied */ ],
};

export const VALID_GUESS_LISTS = {
  4: [ /* SCOWL-50 filtered to length 4 — superset of ANSWER_LISTS[4] */ ],
  5: [ /* SCOWL-50 filtered to length 5 — superset of ANSWER_LISTS[5] */ ],
  6: [ /* SCOWL-50 filtered to length 6 — superset of ANSWER_LISTS[6] */ ],
  7: [ /* SCOWL-50 filtered to length 7 — superset of ANSWER_LISTS[7] */ ],
  8: [ /* SCOWL-50 filtered to length 8 — superset of ANSWER_LISTS[8] */ ],
};
...
|---|---|---|
| `scowl-35.txt` | SCOWL by Kevin Atkinson — MIT-like license | Download the latest SCOWL release tarball from [wordlist.sourceforge.net](http://wordlist.sourceforge.net) or the [SCOWL GitHub mirror](https://github.com/kevina/wordlist). Extract and run `make` with `SCOWL_SIZE=35` to generate the combined word list, **or** use the pre-built `english-words.35` file from the release. |
| `scowl-50.txt` | Same SCOWL release | Same process with `SCOWL_SIZE=50`, or `english-words.50` from the release. |
| `ldnoobwv2-en.txt` | LDNOOBW v2 by @nicktindall | Download `en.txt` from the [LDNOOBW GitHub repo](https://github.com/nicktindall/node-ldnoobw). Rename to `ldnoobwv2-en.txt` and place in the `scripts/` directory. |

Place all three `.txt` source files in `scripts/wordlist-sources/` (gitignored — do not commit the raw source files, only the generated output).
...
git commit -m "chore: regenerate word lists (SCOWL 35/50 + LDNOOBW v2)"
```
...
// Source files expected at: scripts/wordlist-sources/scowl-35.txt
//                           scripts/wordlist-sources/scowl-50.txt
...
// Sources: SCOWL 35 (answers), SCOWL 50 (valid guesses), LDNOOBW v2 (profanity filter)
...
function loadWords(file) {
  return readFileSync(file, 'utf8')
    ...
    .filter(w => /^[A-Z]+$/.test(w)); // alpha only — no hyphens, apostrophes, digits
}

const answerSource = loadWords('scripts/wordlist-sources/scowl-35.txt');
const guessSource  = loadWords('scripts/wordlist-sources/scowl-50.txt');
...
for (const length of [4, 5, 6, 7, 8]) {
  ...
  VALID_GUESS_LISTS[length] = guessSource
    .filter(w => w.length === length);
  // VALID_GUESS_LISTS is a superset of ANSWER_LISTS for each length
...
export function generate(seed, options = {}) {
  ...
  // Step 2: pick word from SCOWL-35 answer list for this length
...
const validGuesses = VALID_GUESS_LISTS[wordLength]; // SCOWL-50 list for this length
...
Player submits guess (Enter key / submit button)
  ...
  ├─ Validate: guess exists in VALID_GUESS_LISTS[wordLength]  (SCOWL-50)
...
| Answer | `<TextField />` | `''` (or pre-populated from cron preview) | auto-uppercased; must be exactly `wordLength` letters; must exist in `ANSWER_LISTS[wordLength]` (SCOWL-35) |
...
As the editor user types in the Answer field, a dropdown of closest matching words appears below the input. This helps editors find valid SCOWL-35 words quickly without needing to consult the word list separately.

**Behaviour:**
- Suggestions are drawn from `ALL_ANSWERS_FLAT` (all lengths merged — SCOWL-35 across lengths 4–8), not filtered to `wordLength` — this lets the editor discover words of any length, after which they may adjust Word Length to match
      ...
      <div class="level-editor-fields">
        ...
        <div class="answer-field-wrapper">
          <TextField
            ...
            hint={`Must be a common ${wordLength}-letter English word (SCOWL-35)`}
...
| # | Question | Status |
|---|---|---|
| 1 | SCOWL-35 for answers, SCOWL-50 for valid guesses | **Decided** — see §5 |
...
| 7 | Autocomplete searches `ALL_ANSWERS_FLAT` (cross-length, SCOWL-35) | **Decided** — see §10.6 |
